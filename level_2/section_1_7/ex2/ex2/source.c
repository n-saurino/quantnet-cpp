...
/*
PROMPT: The following program reads a string with a 30 character maximum. Implement
...
// Adapted from the standard strlen() function in linux kernel
int Length(char str[]) {
	const char* char_ptr; // create character pointer to iterate through char array in memory
	for(char_ptr = str; ;++char_ptr){
		if (*char_ptr == '\0') { // When we find the end of the char arry
			return char_ptr - str; // check the distance between the start of the array and our iterating char pointer
		}
	}
}
